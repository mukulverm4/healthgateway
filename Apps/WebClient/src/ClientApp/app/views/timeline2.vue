    ...
    <b-row class="my-3 fluid justify-content-md-center">
      ...
      <b-col id="timeline" class="col-12 col-md-8 col-lg-6 column-wrapper">
        ...
        <div v-if="!isLoading">
          ...
          <hr />
          <LineComponent :chartdata="timeChartData" :options="chartOptions" />
          <br />
...
export default class TherapeuticClassificationComponent extends Vue {
  ...
  getMedicationList!: ({
    dinList: any
  }: any) => Promise<Dictionary<MedicationResult>>;
  ...
  private categoryChartData: {} | null = null;
  private timeChartData: {} | null = null;
  ...
  private get isLoading(): boolean {
    return this.isMedicationLoading || this.isMedicationDetailLoading;
  }
    ...
    promise
      .then(results => {
        if (results.resultStatus == ResultType.Success) {
          ...
          return this.getMedicationDetails(results.resourcePayload).then(() => {
            // Add the medication entries to the timeline list
            for (let result of results.resourcePayload) {
              let medicationEntry = new MedicationTimelineEntry(result);
  ...
  private prepareMonthlyChart() {
    ...
    for (let i = 0; i < months.length; i++) {
      this.timeChartData.labels.push(moment(months[i]).format("MMM YYYY"));
